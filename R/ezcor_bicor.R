...
ezcor_bicor <- function(
  ...
  ...
    ...
    if (split == FALSE) {
      res <-
        WGCNA::bicor(x = as.numeric(data[, var1]), y = as.numeric(data[, var2]), ...)
      bicor2var_df <-
        data.frame(
          cor = res,
          v1 = var1,
          v2 = var2 ,
          method = ifelse(mad( as.numeric(data[, var2])) == 0, "pearson","bicor"),
          stringsAsFactors = F
        )
      return(bicor2var_df)
    } else{
      ...
      bicor2var <- purrr::map(s, purrr::safely(function(x) {
        sss_sub <- sss[[x]]
        res_bicor <-
          WGCNA::bicor(x = as.numeric(sss_sub[, var1]), y = as.numeric(sss_sub[, var2]), ...)
        res_bicor <-
          data.frame(
            cor = res_bicor,
            v1 = var1,
